...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    #     presentState, presentPath = frontier.pop()
    #     # presentCost = 
    #     if (problem.isGoalState(presentState)):
    #         return presentPath
    ...
    #     else: 
    #         # within for loop
    #         nextNode = problem.getSuccessors(presentState)
    
    _open = util.PriorityQueue()
    _close = []

    initialNode = (problem.getStartState(), [], 0)
    _open.push(initialNode, 0)

    while not _open.isEmpty():
        presentState, allocate, actionCost = _open.pop()
        if(presentState in _close):
            continue
        else:
            _close.append(presentState)                           # adds the starting node so it doesnt loop back
        ...
        if problem.isGoalState(presentState):
            ...
            for nextState,nextAllocate,nextCost in children:
                if nextState not in _close:
                  _open.update((nextState, allocate+[nextAllocate], actionCost+nextCost), actionCost+nextCost+heuristic(nextState, problem))
"""
problem 5
run heuristic building code.
